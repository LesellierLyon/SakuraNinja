...
class GameViewController: UIViewController {
    ...
    // Interface de fin de partie et contrôles
    ...
    // Dimensions de l'écran et positions de spawn (gauche / droite)
    var screenHeight: Int = 0
    var screenWidth: Int = 0
    var positionx: [Int] = []
    var maxSpeed: Int = 0

    // Timers : boucle de jeu et spawn des objets
    var t: Timer!
    var t2: Timer!
    var attente: Int = 0
    var temps: Int = 0

    // Liste active des objets en jeu (fleurs + bombe éventuelle)
    var fleurs: [UIImageView] = []
    var fleur = UIImageView()

    // Vitesse et rotation de chaque objet en vol
    var vitesses: [UIImageView: CGPoint] = [:]
    var rotations: [UIImageView: CGFloat] = [:]

    // Points de contact pour détecter le geste de coupe
    ...
    override func viewDidLoad() {
        ...
        // Initialisation des dimensions et des positions de spawn
        ...
        // Démarrage de la boucle de jeu et du premier spawn
    ...
    /// Planifie l'apparition du prochain objet avec un délai aléatoire selon la difficulté
    func lancerSpawn() {
    ...
    /// Active un objet caché (fleur ou bombe) et le lance depuis un bord de l'écran
    @objc func activation(_ timer: Timer) {
        // 1 chance sur 6 de faire apparaître une bombe
        ...
        for fleur in fleurs {
            if fleur.isHidden == true {
                ...
                // Direction horizontale selon le bord de spawn
                if fleur.frame.origin.x < 0 {
                    vitesses[fleur] = Int(fleur.frame.origin.y) <= (screenHeight + 100) / 2
                        ? CGPoint(x: maxSpeed, y: Int.random(in: 0...2))
                        : CGPoint(x: maxSpeed, y: Int.random(in: -2...0))
                } else {
                    vitesses[fleur] = Int(fleur.frame.origin.y) <= screenHeight / 2
                        ? CGPoint(x: -maxSpeed, y: Int.random(in: 0...2))
                        : CGPoint(x: -maxSpeed, y: Int.random(in: -2...0))
                }

                // Animation de rotation en boucle
                fleur.layer.removeAllAnimations()
                UIView.animate(withDuration: 2, delay: 0, options: [.repeat, .curveLinear], animations: {
                    fleur.transform = fleur.transform.rotated(by: .pi)
                })
                break
            }
        }
    ...
    /// Appelée toutes les 5 ms : déplace les objets et vérifie les coupes
    @objc func boucle(_ timer: Timer) {
        // Déplacement de chaque objet actif
        for fleur in fleurs {
            if fleur.isHidden == false {
                ...
                if (-100...screenWidth + 100).contains(Int(p.x)) {
                    // L'objet est encore dans les limites : on le déplace
                    fleur.frame.origin = p
                } else {
                    // L'objet a quitté l'écran
                    if fleur != bombe {
                        nbVie += 1
                    } else {
                        fleurs.removeFirst()
        ...
        // Détection de coupe par le geste du joueur
    ...
    // MARK: - Interactions avec les objets

    /// Gère la coupe d'une fleur ou d'une bombe par le joueur
    func couperFleur(_ fleur: UIImageView) {
        if fleur == bombe {
            // Affichage de l'explosion et perte immédiate
        ...
        // Animation : la fleur se sépare en deux moitiés qui tombent
        ...
        // Réinitialisation de la fleur coupée
        fleur.layer.removeAllAnimations()
    ...
    /// Arrête la partie, cache les objets et affiche les boutons de fin
    ...
    /// Affiche une petite étoile animée à l'endroit touché par le joueur
    func spawnEtoile(at point: CGPoint) {
        let etoile = UIImageView(image: UIImage(named: "etoile"))
        let size: CGFloat = CGFloat.random(in: 15...30)
        etoile.frame = CGRect(x: point.x - size / 2, y: point.y - size / 2, width: size, height: size)
        view.addSubview(etoile)

        let dx = CGFloat.random(in: -20...20)

        UIView.animate(withDuration: 0.4, delay: 0, options: .curveEaseOut, animations: {
            etoile.frame.origin.y += 60
            etoile.frame.origin.x += dx
            etoile.alpha = 0
            etoile.transform = CGAffineTransform(rotationAngle: .random(in: -.pi ... .pi))
        }, completion: { _ in
